...
io.on("connection", (socket) => {
    ...
    if (targetIsAdmin) {
      ...
      if (sentCount === 0) {
        ...
        io.to(socket.id).emit("callFailed", {
          reason: "No support agents online",
        });
        // Send offline msg
        try {
          const sendSystemMessage = require("./src/utils/sendSystemMessage");
          await sendSystemMessage(
            { app },
            userId,
            `Missed voice call attempt from ${name} (Customer). Admins were offline. 📞`,
            null,
          );
        } catch (e) {}
      }
        ...
        try {
          ...
          if (isAdmin) {
            await sendSystemMessage(
              { app },
              userToCall,
              `You missed a call from support (${name}). 📞`,
              null,
            );
          }
        } catch (msgErr) {}
  ...
  socket.on("answerCall", async (data) => {
    ...
    if (data.callLogId) {
      try {
        const call = await CallLog.findById(data.callLogId)
          .populate("caller", "name")
          .populate("receiver", "name");
        if (call) {
          // Prevent Duplicates
          const existingIndex = activeVoiceCalls.findIndex(
            (c) => c.callLogId === data.callLogId,
          );
          ...
          const callObj = {
            ...
            startTime:
              existingIndex !== -1
                ? activeVoiceCalls[existingIndex].startTime
                : new Date(),
          };
        ...
        console.error("Error populating active call:", e);
      }
    }
  ...
  socket.on("endCall", async (data) => {
    ...
    try {
      const ringingCall = await CallLog.findOne({
        $or: [{ caller: userId }, { caller: data.to }],
        status: "missed",
      }).sort({ createdAt: -1 });
    ...
    if (!broadcastedCancel) {
      console.log(
        `[Socket] No specific ringing call found. Exploring fallback.`,
      );
    ...
    const activeCallIndex = activeVoiceCalls.findIndex(
      (c) =>
        c.caller.id.toString() === userIdStr ||
        c.receiver.id.toString() === userIdStr,
    );
    ...
    if (activeCallIndex !== -1) {
      ...
      console.log(
        `[Socket] Found active call to kill: ${call.caller.name} <-> ${call.receiver.name}`,
      );
      ...
      console.log(
        "[Socket] No active voice call found in memory for this user. Trying DB cleanup.",
      );
    }
    ...
    try {
      ...
      if (activeCall) {
        ...
        if (activeCall.status === "ongoing" && isRecent) {
          ...
          activeCall.duration =
            (activeCall.endTime - activeCall.startTime) / 1000;
  ...
  socket.on("disconnect", async () => {
    ...
    activeVoiceCalls = activeVoiceCalls.filter(
      (c) =>
        c.caller.id.toString() !== userId &&
        c.receiver.id.toString() !== userId,
    );
...
app.use(async (err, req, res, next) => {
  ...
  try {
    ...
    const shouldSkip = skipTerms.some((term) =>
      err.message.toLowerCase().includes(term.toLowerCase()),
    );
