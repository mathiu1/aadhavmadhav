...
const ProductEditPage = () => {
    ...
    useEffect(() => {
        if (updateSuccess) {
            ...
            if (!product || product._id !== productId) {
                ...
                // Handle images: favor images array, fallback to single image
    ...
    }, [dispatch, navigate, productId, product, updateSuccess]);
        ...
        <div className="w-full max-w-[1600px] mx-auto px-4 md:px-0 py-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                ...
                <div className="flex gap-3">
                    ...
                    >
                        {updateLoading ? 'Saving...' : <><FiSave /> Save Changes</>}
                    </button>
                </div>
            </div>
            ...
            {updateError && (
                <div className="bg-red-50 text-red-600 p-6 rounded-[2rem] border border-red-100 flex items-center gap-4 mb-8">
                    <span className="font-bold">{updateError}</span>
                </div>
            )}
            ...
            {isLoading ? (
                ...
                <form onSubmit={submitHandler} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <div className="relative card-select-wrapper">
                                        ...
                                        {/* Custom chevron if needed, but standard select is fine for now or add custom one */}
                                    </div>
                                </div>
                    ...
                    <div className="space-y-8">
                        ...
                        {/* Quick Status (Optional visualization) */}
                    ...
                    {(product?.createdAt || product?.updatedAt) && (
                        <div className="lg:col-span-3 bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100">
                            <h3 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-2">
                                <FiClock className="text-primary" /> Timeline & Audit
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {product?.user && (
                                    <div className="flex items-center gap-4 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-base font-black text-slate-900">{product.user?.name || 'Unknown'}</p>
                                            <p className="text-xs font-medium text-slate-500">{product.user?.email}</p>
                                ...
                                {product?.lastUpdatedBy && (
                                    <div className="flex items-center gap-4 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                        ...
                                        <div>
                                            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Last Updated by</p>
                                            <p className="text-base font-black text-slate-900">{product.lastUpdatedBy?.name || 'Unknown'}</p>
                                            <p className="text-xs font-medium text-slate-500">{product.lastUpdatedBy?.email}</p>
                                            <p className="text-xs font-bold text-slate-400 mt-1">
